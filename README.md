...
The optimal temperature for sleeping is 18-19 °C, and the ideal humidity range is between 40-60%. In this project, the light intensity will not be measured in a unit, such as lux, and can therefore not be compared with an optimal value. Instead, the photoresistor will measure the resistance, which decrease non-linearly as the sensor is exposed to more light. I defined a percentage of darkness based on these resistance values and then determine the desired darkness level for my room at bedtime. The desired darkness percentage was set to 90%.
